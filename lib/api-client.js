...
export async function fetchApi(endpoint, options = {}) {
  ...
  try {
    ...
    // Intercept and normalize dropdown arrays to objects to maintain frontend compatibility
    if (data && data.success && data.data && typeof data.data === 'object') {
      const dropdownKeys = [
        'categories', 'units', 'priceBands', 'gstRates', 'calculationBases', 
        'supplierTypes', 'designations', 'paymentTerms', 'countries', 'states', 
        'cities', 'agentTypes', 'commissionTypes', 'currencies', 'ports', 
        'incoterms', 'shipmentMethods'
      ];
        ...
        if (Array.isArray(data.data[key])) {
          ...
          data.data[key].forEach(item => {
            if (typeof item === 'object' && item !== null) {
              const k = item.id !== undefined ? item.id : item.key;
              const v = item.name !== undefined ? item.name : item.label || item.value || k;
              obj[k] = v;
            } else {
              obj[item] = item;
            }
