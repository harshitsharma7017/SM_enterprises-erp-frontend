...
export default function InvoiceListPage() {
  ...
  const Actions = can('invoice.create') ? (
    <Link href="/finance/invoices/create" className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-md text-sm inline-flex items-center no-underline">
      ...
      <Card title="Final invoices on posted dispatches" variant="primary" actions={Actions}>
        ...
        <form className="flex flex-wrap items-end gap-3 mb-4" onSubmit={(e) => e.preventDefault()}>
          <div className="flex-1 min-w-[180px]">
            ...
            <input type="text" value={filters.search} onChange={(e) => setFilter('search', e.target.value)} placeholder="Invoice, order, PI, dispatch, customer or reference" className={INPUT} />
          </div>
          ...
          <div className="w-32">
            <label className="block text-xs text-gray-500 mb-1">Dispatch No.</label>
            <input type="text" value={filters.dispatch} onChange={(e) => setFilter('dispatch', e.target.value)} className={INPUT} />
          </div>
          <div className="w-32">
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm text-left">
            <thead className="bg-gray-50 text-gray-700">
              <tr>
                ...
                <th className="px-4 py-2 font-medium">PI</th>
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {loading ? (
                <tr><td colSpan="10" className="text-center py-8 text-gray-500">Loading invoices...</td></tr>
              ) : rows.length === 0 ? (
                ...
                <tr key={d.id} className="hover:bg-gray-50">
                  <td className="px-4 py-2"><Link href={`/finance/invoices/${d.id}`} className="font-mono font-semibold text-gray-900 hover:text-blue-600">{d.invoice_no || `Draft #${d.id}`}</Link></td>
                  <td className="px-4 py-2"><CompanyBadge label={d.company_label} code={d.company_code} /></td>
                  <td className="px-4 py-2 text-gray-500 whitespace-nowrap">{formatDate(d.invoice_date)}</td>
                  <td className="px-4 py-2 text-gray-700">{d.buyer_name}</td>
                  <td className="px-4 py-2 font-mono text-xs">{d.oc_num || '—'}</td>
                  <td className="px-4 py-2 font-mono text-xs">{d.pi_no || '—'}</td>
                  <td className="px-4 py-2 font-mono text-xs">{d.dispatch_nos}</td>
                  ...
                  <td className="px-4 py-2"><WorkflowBadge status={d.status} config={COMMERCIAL_STATUS_BADGES} /></td>
                </tr>
              ))}
            </tbody>
          </table>
