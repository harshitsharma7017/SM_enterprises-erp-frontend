              ...
              {loading ? (
                ...
                return (
                  <tr key={m.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-xs">{m.qc_no ? <span className="font-mono">{m.qc_no}</span> : m.issue_no ? <Link href={`/production/material-issues/${m.material_issue_id}`} className="font-mono text-blue-600 hover:underline">{m.issue_no}</Link> : <span className="text-gray-700">{m.reason}</span>}</td>
