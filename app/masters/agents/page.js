...
export default function AgentIndex() {
  ...
  const [filters, setFilters] = useState({ search: '', status: '', agent_type: '', page: 1 });
  const LIMIT = 15;
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      if (filters.agent_type) q.set('agent_type', filters.agent_type);
      q.set('limit', LIMIT);
      q.set('page', filters.page);
      
      const res = await apiClient.get(`/masters/agents?${q}`);
  ...
  const resetFilters = (e) => { e.preventDefault(); setFilters({ search: '', status: '', agent_type: '', page: 1 }); };

  const toggleStatus = async (item) => {
    if (!can('agent.edit')) return;
    try { await apiClient.patch(`/masters/agents/${item.id}/toggle-status`); fetchItems(); } 
    catch (err) { alert('Failed to toggle status: ' + err.message); }
  };

  const deleteItem = async (item) => {
    if (!confirm(`Delete Agent "${item.name}"?`)) return;
    try { await apiClient.delete(`/masters/agents/${item.id}`); fetchItems(); } 
